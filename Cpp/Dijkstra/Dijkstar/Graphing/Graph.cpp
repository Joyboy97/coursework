...
//Step 2:
 ...
 //start index (this is done using the adjacency matrix)
	for(int i=numOfVerts-2;i>=0;i--){
			distances[i]=adjMatrix[start][i];
			if(distances[i]!=100){
				prev[i]=start;
			}
			//cout<<"("<<start<<","<<i<<")"<<adjMatrix[start][i]<<endl;
	}
//Step 3:
...
void Graph::setDistances(int latestVert) { //8 pts
	//int nprev=1000;
	if(latestVert>=0&&latestVert<numOfVerts){
